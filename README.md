...
Obsidian 的 Cmd+P。敲快捷鍵、打幾個字、Enter 跳過去。

**這跟上面的 BM25 是不同用途，刻意分成兩個模組：**

| | 回答的問題 | 比對對象 | 演算法 |
|---|---|---|---|
| `searchDocs`（BM25） | 「答案在哪」 | 文件**內文** | 詞頻排序 |
| `quickSwitch`（模糊） | 「我要跳到那份」 | 文件**標題** | 子序列比對 |

使用者已經知道要去哪的時候，不需要排序演算法，需要的是三個字就跳到位。

中文特別適合模糊比對：英文的 fzf 靠字母子序列（`knwbs` → `knowledge-base`），
中文更直接 —— 每個字本身就是有意義的單位，打「報價」就命中
「報價與折扣授權 SOP」，不需要斷詞也不需要詞典。所以中英文共用同一套邏輯。

排序規則（照重要性）：連續命中 > 命中詞首 > 位置靠前 > 標題短。
標題比不到會退而比路徑（分數打五折），所以「我記得它在 SOP 資料夾」也找得到。
**順序不對就不算命中** —— 寧可少給也不要給錯的。

打 `>` 進入命令模式（VS Code 的慣例）：切換視角、跳頁、重設示範資料。

```tsx
// 面板可以單獨拿出來用
import { CommandPalette, usePaletteHotkey } from '@sales-next/knowledge-base/react';
```
...
1. `seedIfEmpty` 會自動灌入示範資料（5 份公司文件 + 5 場會議沉澱），
   刻意涵蓋四種分類狀態，所以獨立性在畫面上看得出來。
2. **按 ⌘K，打「報價」，Enter** —— 一秒跳到文件。先做這個，
   因為它最快讓人感覺到「這東西是能用的工具，不是 demo 畫面」。
3. 切到「問答」，用內建的建議問題 —— 每一題都能在示範資料裡找到明確答案。
4. **點引文** —— 會跳回原文並 highlight 到正確的段落。這是最有說服力的一下。
5. 切到「匯入」，貼一段 SOP 文字，看**兩套分類器各自**給出結果和理由。
6. 在「瀏覽」切換資料夾／標籤視角，指出兩邊的成員和涵蓋率不一樣，
   再點「未歸檔」「未標記」桶子，說明每套系統各自管自己的未分類。
   立盛物流那筆（抽取稀疏）會在標籤視角消失，是很好的講解點。
7. 頂端「重設示範資料」可以隨時回到乾淨狀態。
...
- **快速切換：中文子序列命中、順序不對不算命中、連續命中排前面、
  大小寫無關、標題優先於路徑、未歸檔不爆掉、反白區間切得回原字串**
...
- 快速切換只比對標題和路徑，不比對內文 —— 記不得標題就改用「問答」。
  也沒有拼音／注音輸入（打 `bj` 不會命中「報價」）。
